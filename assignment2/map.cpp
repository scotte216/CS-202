...
// NOTE: It is assumed the last vertex room in the adjacency list is the final
// room. Therefore if the current room is adj_list[size-1] the player is assumed
// to have reached the end and won.
map::map()
{
    int size = 0;
...
//Returns true or false if the player has reached the last room of the map which
//is assumed to be the last entry in the adjacency list. 
bool map::finished()
{
    if (current == &adj_list[list_size-1])
        return true;
